 ...
 const dateFormat = (date)=>{
  ...
  return (
    <div className='bg-white p-2 rounded-md'>
      ...
      <div className='flex p-2 w-[100%] gap-2'>
         {original.length !==0 ? <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 w-[100%]'>
         {original.map(detail=>detail.map(value=><div key={value._id} className={`${value.booked ? "bg-indigo-500" : "bg-red-500"  }   text-white p-2 rounded-md`}>
          <h4 className='text-xl capitalize'>{value.userName}</h4>
          <ul>
            <li className='capitalize'>Room: {value.name}</li>
            <li className='capitalize'>startDate: {new Date(value.startDate).toLocaleDateString()}</li>
            <li className='capitalize'>endDate: {new Date(value.endDate).toLocaleDateString()}</li>
            <li className='capitalize'>No. of Rooms: {value.no_of_rooms}</li>
            <li className='capitalize'>Total Days: {value.days}</li>
            <li className='capitalize'>Payable Amount:{value.amount}</li>
            <li className='capitalize'>contact no.: {value.userContact}</li>
            <li className='capitalize'>address: {value.userAddress}</li>
